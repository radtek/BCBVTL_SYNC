...
namespace Sync.Api.Controllers
{
    ...
    public class Perso_PersoController : BaseController
    {
        private static readonly ILog log = LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        private PersoService persoService;
        public Perso_PersoController()
        {
            persoService = new PersoService();
        }
        public async Task<IHttpActionResult> NhanDanhSachBTuTTDH(documentList model)
        {
            try
            {
                var rs = await persoService.SaveDSBTuTTDH(model);
                return Ok(rs);
            }
            catch (Exception ex)
            {
