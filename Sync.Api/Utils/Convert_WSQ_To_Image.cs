...
{
    public class Convert_WSQ_To_Image
    {
        /// <summary>
        /// convert wsq to image
        /// </summary>
        /// <param name="base64WSQ"></param>
        /// <returns>
        /// code : lưu base64 trả về
        /// </returns>
        public static ResponseDocumentSync Convert_WSQ(string base64WSQ)
        {
            var result = new ResponseDocumentSync();
            try
            {
                if (!string.IsNullOrEmpty(base64WSQ))
                {
                    var byteArray = Convert.FromBase64String(base64WSQ);

                    WsqDecoder decoder = new WsqDecoder();
                    Bitmap bmp = decoder.Decode(byteArray);
            ...
            {
                return new ResponseDocumentSync { success = false, errorMessage = "Lỗi xử lý dữ liệu." };
            }
        }
    }
